...
DELETE FROM admin_actions WHERE admin_user_id IN (
    SELECT id FROM users WHERE email LIKE 'perftest%@example.com'
);
...
DELETE FROM point_transactions WHERE user_id IN (60, 61, 62);

-- Delete login attempts for test users
DELETE FROM login_attempts WHERE email LIKE 'perftest%@example.com';

-- Now delete parent records
DELETE FROM auths WHERE email LIKE 'perftest%@example.com';
